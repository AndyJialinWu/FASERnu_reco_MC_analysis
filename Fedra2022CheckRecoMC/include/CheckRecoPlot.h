...
CheckRecoPlot::CheckRecoPlot(){
    ...
    for(int i=0; i<2; i++){
        ...
        h2_IP_dz[i] = new TH2D("h2_IP_dz_" + str_TrkType[i], "IP versus #Delta z", 1000, 0, 1000000, 1000, 0, 10000);
        h2_IP_dz[i]->GetXaxis()->SetTitle("#Delta z [#mu m]");
    ...
    h2_preco_ptrue = new TH2D("h2_preco_ptrue", "P_{reco} versus P_{true}", 100, 0, 100, 100, 0, 100);
